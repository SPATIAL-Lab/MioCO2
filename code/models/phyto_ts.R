model {
  ...
  for(i in 1:n.obs){
    ...
    len.lith[i] = lith.m * (rm[si[i], ai[i]] * 1e6) + lith.b # in um
    
    # Calculate cell carbon content
    cell.vol.um[i] = 4 / 3 * 3.141593 * ((rm[si[i], ai[i]] * 1e6) ^ 3) # in um^3
...
#    mui[i] = max(mui.raw[i], 1*10^-6)
  ...
  for(i in 1:length(si)){
    for(j in 2:length(ai)){
      ...
      ## Salinity (ppt)
      sal[i, j] ~ dgamma(sal.m[i, j - 1] ^ 2 / sal.v, sal.m[i, j - 1] / sal.v)
      
      ## Concentration of phosphate (PO4; umol/kg)
      po4[i, j] ~ dgamma(po4.m[i, j - 1] ^ 2 / po4.v, po4.m[i, j - 1] / po4.v)
      
      ## Mean cell radius (m)
      rm[i, j] ~ dgamma(rm.m[i, j - 1] ^ 2 / rm.v, rm.m[i, j - 1] / rm.v)
    }
    ...
    po4[i, 1] ~ dgamma(po4.pri[i, 1] ^ 2 / po4.pri[i, 2], 
                       po4.pri[i, 1] / po4.pri[i, 2])
    
    rm[i, 1] ~ dgamma(1.5e-6 ^ 2 / 0.5e-6, 1.5e-6 / 0.5e-6)
  }
